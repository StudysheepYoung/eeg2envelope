...
import numpy as np
import pandas as pd  # 如果已import可省略
...
def main():
    ...
    all_val_metrics = []  # 新增：用于收集所有epoch的验证集结果
    ...
    for epoch in range(args.epoch):
        ...
        if (epoch + 1) % args.eval_interval == 0:
            ...
            val_loss = 0
            val_metric = 0
            val_metric_list = []  # 新增：用于收集每个batch的metric
            ...
            with torch.no_grad():
                for val_inputs, val_labels, val_sub_id in val_dataloader:
                    ...
                    batch_metric = pearson_metric(val_outputs, val_labels).mean().item()
                    val_metric += batch_metric
                    val_metric_list.append(batch_metric)  # 新增：收集每个batch的metric

                val_loss /= len(val_dataloader)
                val_metric /= len(val_dataloader)
                # val_metric = val_metric.mean()  # 不再对metric取均值

                # 只在主进程上打印和记录日志
                if is_main_process:
                    Logger(f'|-Validation-|Step:{global_step}: loss:{val_loss.mean().item():.3f} metric(均值):{val_metric:.3f}')
                    if writer:
                    ...
                    if (epoch + 1) % 50 == 0:
                        ...
                        plt.boxplot(val_metric_list)
                        plt.title(f'Validation Pearson Metric Distribution (Epoch {epoch+1})')
                        plt.ylabel('Pearson Metric')
                        plt.grid(True)
                        boxplot_save_path = os.path.join(save_path, f'val_metric_boxplot_epoch_{epoch+1}.png')
                    ...
                    if writer:
                        ...
                        plt.ylabel('Pearson Metric')
                        plt.grid(True)
                        writer.add_figure('验证集Pearson分布箱线图', fig, global_step)
                        plt.close(fig)

            # 新增：收集本epoch所有batch的结果
            for idx, metric in enumerate(val_metric_list):
                all_val_metrics.append({
                    'epoch': epoch + 1,
                    'batch_idx': idx,
                    'pearson_metric': metric
                })
        ...
        # print(f'验证集样本数: {len(val_dataloader)}')

    # 训练循环后（for epoch ... 之后）
    if is_main_process:
        all_val_metrics_df = pd.DataFrame(all_val_metrics)
        all_val_metrics_df.to_csv(os.path.join(save_path, 'all_val_metrics.csv'), index=False)
