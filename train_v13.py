...
def setup_distributed(args):
    ...
    return use_ddp, local_rank, world_size, rank


def setup_environment(args, use_ddp, local_rank, rank):
    # 设置随机种子以确保实验可重复性
    ...
    return input_length, device, data_folder, features, save_path, is_main_process, writer
...
def Logger(content, is_main_process):
    if is_main_process:
        print(content)
...
def setup_model(args, device, use_ddp, local_rank):
    # 设置模型
...
def get_viz_sample(test_dataloader, args, device, is_main_process):
    # 为可视化获取固定的测试样本
    viz_sample = None
    if is_main_process:
...
def train_step(model, inputs, labels, sub_id, optimizer, args, device, writer, is_main_process, global_step):
    optimizer.zero_grad()
    ...
    return loss, l_mse, l_p


def log_training_progress(epoch, args, step, iter_per_epoch, loss, optimizer, start_time, writer, is_main_process, global_step, l_mse, l_p):
...
def evaluate_model(model, dataloader, device, split_name):
    model.eval()
    total_loss = 0
    total_metric = 0
    
    with torch.no_grad():
        for inputs, labels, sub_id in dataloader:
            inputs = inputs.squeeze(0).to(device)
            labels = labels.squeeze(0).to(device)
            sub_id = sub_id.to(device)

            outputs = model(inputs, sub_id)
            total_loss += pearson_loss(outputs, labels).mean()
            total_metric += pearson_metric(outputs, labels).mean()

        total_loss /= len(dataloader)
        total_metric /= len(dataloader)
        total_metric = total_metric.mean()
        
    return total_loss.mean().item(), total_metric.item()


def visualize_results(model, viz_sample, epoch, save_path, writer, global_step, is_main_process):
    if is_main_process and viz_sample is not None:
        viz_inputs, viz_labels, viz_sub_id = viz_sample
        with torch.no_grad():  # 使用no_grad上下文管理器确保不计算梯度
            viz_outputs = model(viz_inputs, viz_sub_id)
        
        # 转换为numpy以便绘图
        viz_outputs_np = viz_outputs[0].detach().cpu().numpy()  # 添加detach()
        viz_labels_np = viz_labels[0].detach().cpu().numpy()  # 添加detach()
...
def save_model(model, optimizer, learning_rate, epoch, save_path, global_step, use_ddp, local_rank, is_main_process):
    if is_main_process:
...
def train_model(model, optimizer, scheduler, train_dataloader, val_dataloader, test_dataloader, 
               viz_sample, args, device, is_main_process, writer, save_path, use_ddp, local_rank):
    # 计算每个epoch的总步数
    ...
    for epoch in range(args.epoch):
        ...
        for step, (inputs, labels, sub_id) in enumerate(train_dataloader):
            ...
            # 训练一步
            loss, l_mse, l_p = train_step(model, inputs, labels, sub_id, optimizer, args, device, 
                                         writer, is_main_process, global_step)

            # 记录训练进度
            if step % args.writing_interval == 0:
                log_training_progress(epoch, args, step, iter_per_epoch, loss, optimizer, 
                                     start_time, writer, is_main_process, global_step, l_mse, l_p)
            
        # 修改为基于epoch的评估
        if (epoch + 1) % args.eval_interval == 0:
            # 验证集评估
            val_loss, val_metric = evaluate_model(model, val_dataloader, device, "Validation")
            
            # 测试集评估
            test_loss, test_metric = evaluate_model(model, test_dataloader, device, "Test")
            
            # 可视化结果
            visualize_results(model, viz_sample, epoch, save_path, writer, global_step, is_main_process)
                
            # 只在主进程上打印和记录日志
            if is_main_process:
                Logger(f'|-Validation-|Step:{global_step}: loss:{val_loss:.3f} metric:{val_metric:.3f}', is_main_process)
                if writer:
                    writer.add_losses("Loss", "Validation", val_loss, global_step)
                    writer.add_losses("Pearson", "Validation", val_metric, global_step)
                
                Logger(f'|-Test-|Step:{global_step}: loss:{test_loss:.3f} metric:{test_metric:.3f}', is_main_process)
                if writer:
                    writer.add_losses("Loss", "Test", test_loss, global_step)
                    writer.add_losses("Pearson", "Test", test_metric, global_step)
        ...
        if (epoch + 1) % args.saving_interval == 0:
            learning_rate = optimizer.param_groups[0]["lr"]
            save_model(model, optimizer, learning_rate, epoch, save_path, global_step, 
                      use_ddp, local_rank, is_main_process)
...
if __name__ == '__main__':
    ...
    # 设置分布式训练环境
    use_ddp, local_rank, world_size, rank = setup_distributed(args)
    
    # 设置环境变量
    input_length, device, data_folder, features, save_path, is_main_process, writer = setup_environment(
        args, use_ddp, local_rank, rank)
    ...
    # 设置模型、优化器和调度器
    model, optimizer, scheduler = setup_model(args, device, use_ddp, local_rank)
    ...
    # 获取用于可视化的测试样本
    viz_sample = get_viz_sample(test_dataloader, args, device, is_main_process)
    
    # 训练模型
    train_model(model, optimizer, scheduler, train_dataloader, val_dataloader, test_dataloader, 
               viz_sample, args, device, is_main_process, writer, save_path, use_ddp, local_rank) 